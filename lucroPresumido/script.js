...
function calculoIcms() {
  ...
  let valorTransformadoIcms =
    parseFloat(faturamentoIcms.value.replace(/\./g, "").replace(",", ".")) || 0;
...
function icmsCred() {
  ...
  let creditoIcmsTransformado =
    parseFloat(valorCreditoIcms.value.replace(/\./g, "").replace(",", ".")) ||
    0;
  let resultadoCreditoIcms = document.getElementById("cre");

  let resultadoIcmsCredito = creditoIcmsTransformado * 0.2;
...
function calculoIss() {
  ...
  let valorTransformadoIss =
    parseFloat(faturamentoIss.value.replace(/\./g, "").replace(",", ".")) || 0;
...
function calculoPis() {
  ...
  let valorTransformadoPis =
    parseFloat(faturamentoPis.value.replace(/\./g, "").replace(",", ".")) || 0;
...
function calculoCofins() {
  ...
  let valorTransformadoCofins =
    parseFloat(faturamentoCofins.value.replace(/\./g, "").replace(",", ".")) ||
    0;
...
function calculoIrpj() {
  ...
  let valorTransfomadoTrimestre =
    parseFloat(
      faturamentoTrimestral.value.replace(/\./g, "").replace(",", "."),
    ) || 0;
  ...
  if (atividadeIrpj === "comercio") {
    baseCalculoIrpj =
      valorTransfomadoTrimestre * impostosLucroPreumido.presunsaoComercio;
  } else if (atividadeIrpj === "servico") {
    baseCalculoIrpj =
      valorTransfomadoTrimestre * impostosLucroPreumido.presunsaoServico;
  }

  valorIrpj = baseCalculoIrpj * impostosLucroPreumido.IRPJ;

  if (baseCalculoIrpj > 60000) {
    adicionalIrpj = (baseCalculoIrpj - 60000) * 0.1;
  }
...
function calculoCsll() {
  ...
  let valorTransformadoCsll =
    parseFloat(
      faturamentoTrimestralCsll.value.replace(/\./g, "").replace(",", "."),
    ) || 0;
  ...
  if (atividadeCsll === "comercio") {
    baseCsll =
      valorTransformadoCsll * impostosLucroPreumido.presunsaoComercioCsll;
...
function calcularIsento() {
  ...
  let valorTransformadoIsento =
    parseFloat(faturamentoIsento.value.replace(/\./g, "").replace(",", ".")) ||
    0;
